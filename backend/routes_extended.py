...
async def create_albaran(
    ...
    # (que ya tienen el descuento por línea aplicado)
    if kilos_destare == 0:
        items_sin_destare = [item for item in albaran_dict.get("items", []) if not item.get("es_destare")]
        albaran_dict["total_albaran"] = round(sum(item.get("total", 0) for item in items_sin_destare), 2)
...
async def update_albaran(
    ...
    if contrato and es_compra and contrato.get("tipo") == "Compra":
        ...
        if descuento_porcentaje > 0 and kilos_brutos > 0:
            ...
            if descuento_linea > 0:
                update_data["total_albaran"] = round(importe_sin_dto * (1 - descuento_linea / 100), 2)
            else:
                update_data["total_albaran"] = round(importe_sin_dto, 2)
