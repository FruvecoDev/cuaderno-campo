...
@router.get("/tratamientos/export/pdf")
async def export_tratamientos_pdf(
    campana: Optional[str] = None,
    current_user: dict = Depends(get_current_user)
):
    """Export tratamientos to PDF"""
    from fastapi.responses import StreamingResponse
    import io as _io
    ...
    tratamientos_list = await tratamientos_collection.find(query).sort("fecha_tratamiento", -1).to_list(5000)
    tratamientos_raw = serialize_docs(tratamientos_list)
    ...
    title_style = ParagraphStyle('Title', parent=styles['Heading1'], fontSize=16, textColor=colors.HexColor('#1565C0'), alignment=1, spaceAfter=8*mm)
    elements.append(Paragraph("Informe de Tratamientos - FRUVECO", title_style))
    ...
    table_data = [["Fecha", "Tipo", "Subtipo", "Producto", "Dosis", "Parcelas", "Sup. (ha)", "Campana"]]
    for t in tratamientos_raw:
        table_data.append([
            t.get("fecha_tratamiento", "")[:10] if t.get("fecha_tratamiento") else "",
            t.get("tipo_tratamiento", "")[:15], t.get("subtipo", "")[:15],
            t.get("producto_fitosanitario_nombre", "")[:25],
            f"{t.get('producto_fitosanitario_dosis', '')} {t.get('producto_fitosanitario_unidad', '')}".strip()[:15],
            str(len(t.get("parcelas_ids", []))),
            f"{t.get('superficie_aplicacion', 0):.2f}",
            t.get("campana", "")
        ])

    col_widths = [25*mm, 25*mm, 25*mm, 45*mm, 25*mm, 20*mm, 20*mm, 22*mm]
    ...
    pdf.build(elements)
    output.seek(0)
    filename = f"tratamientos_{datetime.now().strftime('%Y%m%d')}.pdf"
