      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem', backdropFilter: 'blur(4px)' }} onClick={() => { setShowForm(false); setEditingId(null); resetForm(); setActiveTab('general'); }}>
          <div className="card" style={{ maxWidth: '960px', width: '100%', height: '85vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative', padding: '2rem', borderRadius: '12px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '0', marginBottom: '1.5rem', borderBottom: '2px solid hsl(var(--border))', overflowX: 'auto', overflowY: 'hidden', flexShrink: 0 }} className="proveedores-tabs">
              {[{key:'general',label:'Datos Generales',icon:<Users size={14}/>},{key:'gestion',label:'Datos Gestion',icon:<CreditCard size={14}/>},{key:'documentos',label:'Documentos',icon:<FileText size={14}/>},{key:'certificaciones',label:'Certificaciones',icon:<Award size={14}/>},{key:'centros',label:'Centros Descarga',icon:<Truck size={14}/>},{key:'agentes',label:'Agentes/Cultivo',icon:<Users size={14}/>},{key:'historial',label:'Historial',icon:<Eye size={14}/>}].map(tab=>(<button key={tab.key} type="button" data-testid={`tab-${tab.key}`} onClick={()=>{setActiveTab(tab.key);if(tab.key==='historial'&&editingId)fetchChangelog(editingId);}} style={{display:'flex',alignItems:'center',gap:'0.35rem',padding:'0.6rem 0.85rem',fontSize:'0.78rem',fontWeight:activeTab===tab.key?'700':'500',color:activeTab===tab.key?'hsl(var(--primary))':'hsl(var(--muted-foreground))',background:'none',border:'none',borderBottom:activeTab===tab.key?'2px solid hsl(var(--primary))':'2px solid transparent',cursor:'pointer',whiteSpace:'nowrap',marginBottom:'-2px',flexShrink:0}}>{tab.icon}{tab.label}</button>))}
            </div>
      ...
      {showTiposIvaManager && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, padding: '1rem', backdropFilter: 'blur(4px)' }} onClick={() => setShowTiposIvaManager(false)}>
          <div className="card" style={{ maxWidth: '450px', width: '100%', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
              <input type="text" className="form-input" placeholder="Nombre (ej: Reducido 10%)" value={nuevoTipoIva.nombre} onChange={(e) => setNuevoTipoIva({...nuevoTipoIva, nombre: e.target.value})} style={{ flex: 2 }} />
              <input type="text" className="form-input" placeholder="%" value={nuevoTipoIva.valor} onChange={(e) => setNuevoTipoIva({...nuevoTipoIva, valor: e.target.value})} style={{ flex: 1 }} />
            ...
            <div style={{ maxHeight: '300px', overflow: 'auto' }}>
              {tiposIva.map(t => (
                <div key={t._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0.75rem', borderRadius: '6px', marginBottom: '0.35rem', background: 'hsl(var(--muted) / 0.3)', border: '1px solid hsl(var(--border))' }}>
                  <span style={{ fontSize: '0.9rem' }}>{t.nombre} ({t.valor}%)</span>
