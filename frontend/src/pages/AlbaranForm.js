...
import { ArrowLeft, PlusCircle, Package, Printer, Download, X, FileText } from 'lucide-react';
import { usePermissions, usePermissionError } from '../utils/permissions';
import { useAuth } from '../contexts/AuthContext';
import AlbaranPreciosCalidad from '../components/albaranes/AlbaranPreciosCalidad';
import AlbaranLineItem from '../components/albaranes/AlbaranLineItem';
import '../App.css';
      ...
      <form onSubmit={handleSubmit} style={{ flex: 1, overflow: 'auto', minHeight: 0, paddingRight: '0.5rem' }}>
          ...
          {selectedContrato && (
            <div style={{ 
              ...
              {isGuisante && preciosCalidad.length > 0 && (
                <AlbaranPreciosCalidad preciosCalidad={preciosCalidad} />
              )}
            </div>
          )}
        </div>
        ...
        <div style={{ marginBottom: '1.25rem' }}>
          ...
          {formData.items.map((item, index) => (
            <AlbaranLineItem
              key={item.id_item || item.carga_id || `line-${index}`}
              item={item}
              index={index}
              isGuisante={isGuisante}
              preciosCalidad={preciosCalidad}
              articulosCatalogo={articulosCatalogo}
              articuloSearch={articuloSearch}
              activeSearchIndex={activeSearchIndex}
              canRemove={formData.items.filter(i => !i.es_destare).length > 1}
              formatCantidad={formatCantidad}
              parseSpanishNumber={parseSpanishNumber}
              calculateItemTotal={calculateItemTotal}
              getPrecioByTenderometria={getPrecioByTenderometria}
